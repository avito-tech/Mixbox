...
public func GREYAssertEqual(
    _ left: @autoclosure () -> AnyObject?,
    _ right: @autoclosure () -> AnyObject?,
    reason: String)
{
    GREYAssert(left() === right(), reason, details: "Expected left term to be equal to right term")
}

public func GREYAssertNotEqual(
    _ left: @autoclosure () -> AnyObject?,
    _ right: @autoclosure () -> AnyObject?,
    reason: String)
{
...
public func GREYAssertEqualObjects<T: Equatable>(
...
public func GREYAssertNotEqualObjects<T: Equatable>(
    _ left: @autoclosure () -> T?,
    _ right: @autoclosure () -> T?,
    reason: String)
{
...
private func GREYSetCurrentAsFailable() {
    ...
    let threadValueForKeyFailureHandlerKey = Thread.current.threadDictionary.value(
        forKey: kGREYFailureHandlerKey
    )
    
    guard let greyFailureHandler = threadValueForKeyFailureHandlerKey as? GREYFailureHandler else {
        return
    }
...
final class EarlGrey {
    ...
    class func setFailureHandler(
        handler: GREYFailureHandler?,
        file: StaticString = #file,
        line: UInt = #line)
    {
        return EarlGreyImpl.invoked(fromFile: file.description, lineNumber: line)
    ...
    class func handle(
        exception: GREYFrameworkException,
        details: String,
        file: StaticString = #file,
        line: UInt = #line)
    {
    ...
    @discardableResult class func rotateDeviceTo(
