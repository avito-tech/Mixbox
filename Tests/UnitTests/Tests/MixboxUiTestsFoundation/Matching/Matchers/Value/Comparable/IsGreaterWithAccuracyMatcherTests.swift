...
class IsGreaterWithAccuracyMatcherTests: BaseMatcherTests {
    func test___match___mismatches___if_value_is_less() {
        assertMismatches(
            ...
            description:
            """
            value (with accuracy ±0.1) is not greater than '2.0', actual value: '1.0'
            """
        )
    }
    
    func test___match___mismatches___if_value_is_less_but_within_accuracy() {
        assertMismatches(
            matcher: matcher(value: 2, accuracy: 0.1),
            value: 1.9,
            description:
            """
            value (with accuracy ±0.1) is not greater than '2.0', actual value: '1.9'
            """
        )
    }
    ...
    func test___match___mismatches___if_value_is_equal_and_accuracy_is_zero() {
        assertMismatches(
            ...
            description:
            """
            value (with accuracy ±0.0) is not greater than '2.0', actual value: '2.0'
            """
        )
    }
    ...
    private func matcher(value: Double, accuracy: Double) -> Matcher<Double> {
        IsGreaterWithAccuracyMatcher(value, accuracy: accuracy)
    }
}
