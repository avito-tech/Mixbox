...
class BaseActionTestCase: TestCase {
        ...
        errorMessageSuffix: () -> String = { "" })
    {
        ...
        if resetViewsForCurrentActionSpecification {
            setViews(
                showInfo: true,
                viewNames: [actionSpecification.elementId]
            )
        }
        ...
        actionSpecification: ActionSpecification<T>)
    {
        ...
        checkActionWaits(
            actionSpecification: actionSpecification,
            uiBeforeDelay: Array(repeating: specForUi, count: 0),
            uiAfterDelay: Array(repeating: specForUi, count: 1)
        )
    }
        ...
        actionSpecification: ActionSpecification<T>)
    {
        ...
        checkActionWaits(
            actionSpecification: actionSpecification,
            uiBeforeDelay: Array(repeating: specForUi, count: 2),
            uiAfterDelay: Array(repeating: specForUi, count: 1)
        )
    }
    
    func checkActionWaits<T: Element>(
        actionSpecification: ActionSpecification<T>,
        uiBeforeDelay: [AnyActionSpecification],
        uiAfterDelay: [AnyActionSpecification])
    {
        ...
        dispatchQueue.sync { [weak self] in
            self?.setViews(showInfo: true, actionSpecifications: uiBeforeDelay)
        }
        
        // Set up UI so action will stop failing after delay
        dispatchQueue.async { [weak self] in
            Thread.sleep(forTimeInterval: delay)
            self?.setViews(showInfo: true, actionSpecifications: uiAfterDelay)
        }
        ...
        viewNames: [String])
    {
        let error = ipcClient.callOrFail(
            ...
            arguments: ActionsTestsViewModel(
                showInfo: showInfo,
                viewNames: viewNames
            )
        )
